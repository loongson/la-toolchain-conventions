...
|1.2
|add 32-bit LoongArch related conventions.
|====
...
|-march=
|`native` `la64v1.0` `la64v1.1` `loongarch64` `la464` `la664` `la32v1.0` `la32rv1.0`
|Select the target architecture, i.e.
the basic collection of enabled <<feature-subsets,ISA feature subsets>>.
|

|-mtune=
|`native` `generic` `loongarch64` `la464` `la664` `loongarch32`
|Select the target microarchitecture, default to the value of `-march=`
or an implementation-defined default if that is not possible.
|
...
|LoongArch32 reduced base architecture
|`la32r`
|ISA defined in __LoongArch Architecture 32-bit Reduced Reference Manual__ v1.04.
|===
...
.Non-LoongArch-specific C/C++ built-in macros related to the ABI
[%header,cols="^1,^3"]
|===
|Name
|Description
